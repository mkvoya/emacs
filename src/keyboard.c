...
/* A vector holding top bar items while they are parsed in function
   top_bar_items. Each item occupies TOP_BAR_ITEM_NSCLOTS elements
   in the vector.  */

static Lisp_Object top_bar_items_vector;


/* A vector holding the result of parse_top_bar_item.  Layout is like
   the one for a single item in top_bar_items_vector.  */

static Lisp_Object top_bar_item_properties;

/* Next free index in top_bar_items_vector.  */

static int ntop_bar_items;

/* Function prototypes.  */

static void init_top_bar_items (Lisp_Object);
static void process_top_bar_item (Lisp_Object, Lisp_Object, Lisp_Object,
				   void *);
static bool parse_top_bar_item (Lisp_Object, Lisp_Object);
static void append_top_bar_item (void);


/* Return a vector of top bar items for keymaps currently in effect.
   Reuse vector REUSE if non-nil.  Return in *NITEMS the number of
   top bar items found.  */

Lisp_Object
top_bar_items (Lisp_Object reuse, int *nitems)
{
  Lisp_Object *maps;
  Lisp_Object mapsbuf[3];
  ptrdiff_t nmaps, i;
  ...
  *nitems = 0;
  ...
  /* Initialize top_bar_items_vector and protect it from GC.  */
  init_top_bar_items (reuse);
  ...
  if (!NILP (Voverriding_local_map_menu_flag)
    ...
    {
      /* No, so use major and minor mode keymaps and keymap property.
	 Note that top-bar bindings in the local-map and keymap
	 properties may not work reliably, as they are only
	 recognized when the top-bar (or mode-line) is updated,
	 which does not normally happen after every command.  */
      ptrdiff_t nminor = current_minor_maps (NULL, &tmaps);
      SAFE_NALLOCA (maps, 1, nminor + 4);
      nmaps = 0;
  ...
  for (i = nmaps - 1; i >= 0; --i)
	...
	keymap = get_keymap (access_keymap (maps[i], Qtop_bar, 1, 0, 1), 0, 1);
	if (CONSP (keymap))
	  map_keymap (keymap, process_top_bar_item, Qnil, NULL, 1);
      }

  Vinhibit_quit = oquit;
  *nitems = ntop_bar_items / TOP_BAR_ITEM_NSLOTS;
  SAFE_FREE ();
  return top_bar_items_vector;
}
...
static void
process_top_bar_item (Lisp_Object key, Lisp_Object def, Lisp_Object data, void *args)
{
  int i;
  ...
  if (EQ (def, Qundefined))
    {
      /* If a map has an explicit `undefined' as definition,
	 discard any previously made item.  */
      for (i = 0; i < ntop_bar_items; i += TOP_BAR_ITEM_NSLOTS)
	{
	  Lisp_Object *v = XVECTOR (top_bar_items_vector)->contents + i;

	  if (EQ (key, v[TOP_BAR_ITEM_KEY]))
	    {
	      if (ntop_bar_items > i + TOP_BAR_ITEM_NSLOTS)
		memmove (v, v + TOP_BAR_ITEM_NSLOTS,
			 ((ntop_bar_items - i - TOP_BAR_ITEM_NSLOTS)
			  * word_size));
	      ntop_bar_items -= TOP_BAR_ITEM_NSLOTS;
	      break;
	    }
	}
    }
  else if (parse_top_bar_item (key, def))
    /* Append a new top bar item to top_bar_items_vector.  Accept
       more than one definition for the same key.  */
    append_top_bar_item ();
}

/* Access slot with index IDX of vector top_bar_item_properties.  */
#define PROP(IDX) AREF (top_bar_item_properties, (IDX))
static void
set_prop_top_bar (ptrdiff_t idx, Lisp_Object val)
{
  ASET (top_bar_item_properties, idx, val);
}


/* Parse a top bar item specification ITEM for key KEY and return the
   result in top_bar_item_properties.  Value is false if ITEM is
   invalid.
   ...
   BINDING is the top bar item's binding.  Top-bar items with keymaps
   as binding are currently ignored.
   ...
   FORM is evaluated and specifies whether the top bar item is
   enabled or disabled.

   - `:visible FORM'

   FORM is evaluated and specifies whether the top bar item is visible.
   ...
   IMAGES is either a single image specification or a vector of four
   image specifications.  See enum top_bar_item_images.

   - `:help HELP-STRING'.

   Gives a help string to display for the top bar item.

   - `:label LABEL-STRING'.

   A text label to show with the top bar button if labels are enabled.  */

static bool
parse_top_bar_item (Lisp_Object key, Lisp_Object item)
{
  Lisp_Object filter = Qnil;
  Lisp_Object caption;
  int i;
  ...
  /* Definition looks like `(menu-item CAPTION BINDING PROPS...)'.
     Rule out items that aren't lists, don't start with
     `menu-item' or whose rest following `top-bar-item' is not a
     list.  */
  if (!CONSP (item))
    return 0;
  ...
  /* Create top_bar_item_properties vector if necessary.  Reset it to
     defaults.  */
  if (VECTORP (top_bar_item_properties))
    {
      for (i = 0; i < TOP_BAR_ITEM_NSLOTS; ++i)
	set_prop_top_bar (i, Qnil);
    }
  else
    top_bar_item_properties = make_nil_vector (TOP_BAR_ITEM_NSLOTS);

  /* Set defaults.  */
  set_prop_top_bar (TOP_BAR_ITEM_KEY, key);
  set_prop_top_bar (TOP_BAR_ITEM_ENABLED_P, Qt);
  ...
  set_prop_top_bar (TOP_BAR_ITEM_CAPTION, caption);
  ...
  if (!CONSP (item))
    {
      if (menu_separator_name_p (SSDATA (caption)))
	{
	  set_prop_top_bar (TOP_BAR_ITEM_ENABLED_P, Qnil);
	  set_prop_top_bar (TOP_BAR_ITEM_SELECTED_P, Qnil);
	  set_prop_top_bar (TOP_BAR_ITEM_CAPTION, Qnil);
	  return 1;
	}
      return 0;
    }
  ...
  /* Store the binding.  */
  set_prop_top_bar (TOP_BAR_ITEM_BINDING, XCAR (item));
  item = XCDR (item);
  ...
  FOR_EACH_TAIL (item)
      ...
      if (EQ (ikey, QCenable))
	{
	  ...
	  if (!NILP (Venable_disabled_menus_and_buttons))
	    ...
	    set_prop_top_bar (TOP_BAR_ITEM_ENABLED_P, value);
	}
      else if (EQ (ikey, QCvisible))
	{
	  /* `:visible FORM'.  If got a visible property and that
	  ...
	  if (NILP (menu_item_eval_property (value)))
        ...
        /* `:help HELP-STRING'.  */
        set_prop_top_bar (TOP_BAR_ITEM_HELP, value);
      else if (EQ (ikey, QCfilter))
	/* ':filter FORM'.  */
	filter = value;
      else if (EQ (ikey, QCbutton) && CONSP (value))
	{
	  ...
	  if (EQ (type, QCtoggle) || EQ (type, QCradio))
	    {
	      set_prop_top_bar (TOP_BAR_ITEM_SELECTED_P, selected);
	    }
	}
    }
  ...
  /* If got a filter apply it on binding.  */
  if (!NILP (filter))
    set_prop_top_bar (TOP_BAR_ITEM_BINDING,
	      (menu_item_eval_property
	       (list2 (filter,
		       list2 (Qquote,
			      PROP (TOP_BAR_ITEM_BINDING))))));

  /* See if the binding is a keymap.  Give up if it is.  */
  if (CONSP (get_keymap (PROP (TOP_BAR_ITEM_BINDING), 0, 1)))
    return 0;

  /* Enable or disable selection of item.  */
  if (!EQ (PROP (TOP_BAR_ITEM_ENABLED_P), Qt))
    set_prop_top_bar (TOP_BAR_ITEM_ENABLED_P,
	      menu_item_eval_property (PROP (TOP_BAR_ITEM_ENABLED_P)));

  /* Handle radio buttons or toggle boxes.  */
  if (!NILP (PROP (TOP_BAR_ITEM_SELECTED_P)))
    set_prop_top_bar (TOP_BAR_ITEM_SELECTED_P,
	      menu_item_eval_property (PROP (TOP_BAR_ITEM_SELECTED_P)));
...
/* Initialize top_bar_items_vector.  REUSE, if non-nil, is a vector
   that can be reused.  */

static void
init_top_bar_items (Lisp_Object reuse)
{
  if (VECTORP (reuse))
    top_bar_items_vector = reuse;
  else
    top_bar_items_vector = make_nil_vector (64);
  ntop_bar_items = 0;
}


/* Append parsed top bar item properties from
   top_bar_item_properties */

static void
append_top_bar_item (void)
{
  ptrdiff_t incr
    = (ntop_bar_items
       - (ASIZE (top_bar_items_vector) - TOP_BAR_ITEM_NSLOTS));

  /* Enlarge top_bar_items_vector if necessary.  */
  if (incr > 0)
    top_bar_items_vector = larger_vector (top_bar_items_vector, incr, -1);

  /* Append entries from top_bar_item_properties to the end of
     top_bar_items_vector.  */
  vcopy (top_bar_items_vector, ntop_bar_items,
	 xvector_contents (top_bar_item_properties), TOP_BAR_ITEM_NSLOTS);
  ntop_bar_items += TOP_BAR_ITEM_NSLOTS;
}
